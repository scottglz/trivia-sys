...
describe('trivia-mafia-analyzer', function() {
   describe('getQuestions', function() { 
      ...
      describe('analyze', function() { 
         it('gets expected results in a monday (adds tuesday\'s question)', function() {
            const body = readFileSync(join(__dirname, '../specfiles/trivia-mafia-email-1.txt'), 'utf-8');
            const date = Date.UTC(2021, 10, 1, 14, 0, 0); // Nov 1 -- monday
         ...
         it('gets expected results in a friday (adds saturday sunday and monday questions)', function() {
            const body = readFileSync(join(__dirname, '../specfiles/trivia-mafia-email-1.txt'), 'utf-8');
